...
assert sum(int(s.get('tests', 0)) for s in suites) == 11, 'Expected 3 regression + 1 account route + 4 account UI + 3 store tests'
assert all(int(s.get(k, 0)) == 0 for s in suites for k in ('failures', 'errors', 'skipped')), 'App device test did not pass'
print('All eight device screenshots, eleven app tests and keyboard measurement verified.')
