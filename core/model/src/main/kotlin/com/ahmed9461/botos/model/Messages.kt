...
data class BotButton(
    val id: String, val label: String, val payload: ActionPayload,
    val enabled: Boolean = true, val style: ButtonVisualStyle = ButtonVisualStyle.DEFAULT,
)
enum class RichMark {
    BOLD, ITALIC, UNDERLINE, STRIKETHROUGH, SPOILER, SUBSCRIPT, SUPERSCRIPT, MARKED, CODE, MATH,
}
data class StyledSpan(
    val text: String, val marks: Set<RichMark> = emptySet(),
    val url: String? = null, val actionId: String? = null,
)
...
data class RichListItem(val label: String = "", val blocks: List<Block>, val checked: Boolean? = null)
data class RichTableCell(
    val content: StyledText, val isHeader: Boolean = false,
    val colspan: Int = 1, val rowspan: Int = 1, val invisible: Boolean = false,
)
enum class MediaKind { PHOTO, VIDEO, ANIMATION, AUDIO, VOICE_NOTE, DOCUMENT, MAP, EMBEDDED, CHAT_LINK }
data class MediaInfo(
    val kind: MediaKind, val title: String = "", val caption: StyledText = StyledText.EMPTY,
    val fileId: Int? = null, val fileName: String = "", val mimeType: String = "",
    val size: Long? = null, val durationSeconds: Int? = null,
    val width: Int? = null, val height: Int? = null, val url: String? = null,
    val latitude: Double? = null, val longitude: Double? = null,
)
sealed interface Block {
    val id: String
    data class Heading(override val id: String, val content: StyledText, val level: Int = 1) : Block {
        constructor(id: String, text: String) : this(id, StyledText.plain(text))
        val text: String get() = content.plainText()
    }
    data class Paragraph(override val id: String, val content: StyledText) : Block {
        constructor(id: String, text: String) : this(id, StyledText.plain(text))
        val text: String get() = content.plainText()
    }
    data class Quote(
        override val id: String, val content: StyledText,
        val credit: StyledText? = null, val expandable: Boolean = false,
    ) : Block {
        constructor(id: String, text: String) : this(id, StyledText.plain(text))
        val text: String get() = content.plainText()
    }
    data class RichQuote(override val id: String, val children: List<Block>, val credit: StyledText = StyledText.EMPTY) : Block
    data class Code(override val id: String, val content: StyledText, val language: String = "") : Block {
        constructor(id: String, text: String) : this(id, StyledText.plain(text))
        val text: String get() = content.plainText()
    }
    ...
    data class RichTable(
        override val id: String, val caption: StyledText = StyledText.EMPTY,
        val rows: List<List<RichTableCell>>, val compact: Boolean = false,
    ) : Block
    ...
    data class Gallery(
        override val id: String, val children: List<Block>,
        val caption: StyledText = StyledText.EMPTY, val slideshow: Boolean = false,
    ) : Block
    data class Unsupported(override val id: String) : Block
}
enum class DeliveryState { NONE, PENDING, SENT, FAILED }
data class BotMessage(
    val id: Long, val chat: ChatKey, val revision: Long, val blocks: List<Block>,
    val outgoing: Boolean = false, val delivery: DeliveryState = DeliveryState.NONE,
    val date: Long = 0L, val forceRtl: Boolean? = null,
    val isFull: Boolean = true, val inlineActions: List<BotButton> = emptyList(),
)
...
data class MessageTimeline(val chat: ChatKey, val messages: List<BotMessage> = emptyList()) {
    ...
    fun resolve(ticket: ActionTicket): BotButton? {
        ...
        val buttons = message.inlineActions.take(256).toMutableList()
        ...
        fun walk(blocks: List<Block>, depth: Int) {
            ...
            blocks.take(ContentLimits.MAX_BLOCKS).forEach {
                ...
                when (it) {
                    ...
                    is Block.RichQuote -> walk(it.children, depth + 1)
...
/** Shared budgets for the whole message, not independently growing nested branches. */
object ContentSafety {
    fun bound(blocks: List<Block>): List<Block> {
        var remaining = ContentLimits.MAX_BLOCKS
        var mediaRemaining = ContentLimits.MAX_MEDIA
        var textRemaining = ContentLimits.MAX_TEXT
        fun text(value: StyledText, max: Int): StyledText {
            var left = minOf(max, textRemaining)
            if (left <= 0) return StyledText.EMPTY
            val spans = buildList {
                for (span in value.spans) {
                    if (left <= 0) break
                    var part = span.text.take(left)
                    if (part.lastOrNull()?.isHighSurrogate() == true) part = part.dropLast(1)
        ...
        fun plain(value: String, max: Int) = text(StyledText.plain(value), max).plainText()
        fun visit(items: List<Block>, depth: Int): List<Block> {
            ...
            for (item in items) {
                ...
                result.add(when (item) {
                    ...
                    is Block.RichQuote -> item.copy(children = visit(item.children, depth + 1), credit = text(item.credit, 512))
                    ...
                    is Block.Bullets -> item.copy(items = item.items.take(100).map { plain(it, 512) })
                    is Block.RichList -> item.copy(items = item.items.take(100).map { entry ->
                        entry.copy(label = plain(entry.label, 64), blocks = visit(entry.blocks, depth + 1))
                    })
                    is Block.Table -> item.copy(
                        headers = item.headers.take(ContentLimits.MAX_COLUMNS).map { plain(it, 256) },
                        rows = item.rows.take(ContentLimits.MAX_ROWS).map { row -> row.take(ContentLimits.MAX_COLUMNS).map { plain(it, 512) } },
                    )
                    is Block.RichTable -> item.copy(
                        caption = text(item.caption, 512),
                        rows = item.rows.take(ContentLimits.MAX_ROWS).map { row -> row.take(ContentLimits.MAX_COLUMNS).map { cell ->
                            cell.copy(content = text(cell.content, 512), colspan = cell.colspan.coerceIn(1, ContentLimits.MAX_COLUMNS), rowspan = cell.rowspan.coerceIn(1, ContentLimits.MAX_ROWS))
                        } },
                    )
                    is Block.Details -> item.copy(title = plain(item.title, 256), children = visit(item.children, depth + 1))
                    ...
                    is Block.Math -> item.copy(expression = plain(item.expression, ContentLimits.MAX_TEXT))
                    is Block.Media -> if (mediaRemaining-- <= 0) Block.Unsupported("media-limit") else item.copy(info = item.info.copy(
                        title = plain(item.info.title, 512), caption = text(item.info.caption, 2048),
                        fileName = item.info.fileName.take(512), mimeType = item.info.mimeType.take(256), url = item.info.url?.take(4096),
                    ))
